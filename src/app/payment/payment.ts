...
export class Payment implements OnInit {
  ...
  loadData(): void {
    this.thanhToanService.getUser().subscribe({
      next: (res: any) => {
        ...
        // 🟩 UPDATED: Kiểm tra đăng nhập từ users.json (có full_name, email, point)
        this.isLoggedIn = !!(res && (res.full_name || res.email || res.id));
      ...
      error: () => {
        console.warn('Không thể tải users.json');
      ...
      this.authService.login({ emailOrPhone: email, password }).subscribe({
        next: (response: any) => {
          if (response && response.user) {
            ...
            this.loadData();
            this.contactForm.patchValue({
              fullName: response.user.full_name || '', // 🟩 UPDATED: Dùng fullName trực tiếp
            ...
            Swal.fire({
              ...
              text: `Xin chào ${response.user.full_name}`,
              timer: 1800,
  ...
  onSelfBookingToggle(checked: boolean): void {
    ...
    if (checked) {
      // 🟩 Tick checkbox → tự động điền dữ liệu từ users.json
      if (this.isLoggedIn && this.currentUser) {
        const fullName = this.currentUser.full_name || '';
        const phone = this.currentUser.phone_number || this.currentUser.phone || '';
